...
- `type` `(string: "aes256-gcm96")` – Specifies the type of key to create. The
  ...
  - `kyber512` - Kyber512 (post-quantum, encryption/decryption, rotation, NIST security level 1)
  - `kyber768` - Kyber768 (post-quantum, encryption/decryption, rotation, NIST security level 3)
  - `kyber1024` - Kyber1024 (post-quantum, encryption/decryption, rotation, NIST security level 5)
...
<!-- kyber-examples:keys -->
#### API usage examples (Kyber)

Create a **Kyber512/Kyber768/Kyber1024** key.
```json
{
  "type": "kyber768"
}
...
$ curl \
  ...
  --request POST \
  --data @payload.json \
  http://127.0.0.1:8200/v1/transit/keys/payments-kyber
```
...
### Sample Kyber request

Pass a **Kyber512/Kyber768/Kyber1024** key type.
```json
{
  "plaintext": "aGVsbG8gcXVhbnR1bSB3b3JsZA==",
  "associated_data": "b3JkZXIjMTIzNA=="
}
```
...
Provide ciphertext from /encrypt and optional Base64 aad (if used during encryption).
```json
{
  "ciphertext": "vault:v1:BASE64_CIPHERTEXT...",
  "aad": "b3JkZXIjMTIzNA=="
}
```
